...
for (path, directories, files) in walk(PATH):
    ...
    for file in files:
        ...
        for index, row in df.iterrows():
            ...
            tracker = BlazeposeDepthai(input_src=file_path, 
                pd_model=args.pd_m,
                lm_model=args.lm_m,
            ...
            # Store the file with skeletal data drawn
            cv2.imwrite(os.path.join(OUTPUT_PATH + "/" + label + "/" + Path(file_path).name), frame)
            ...
            if body is not None:
                ...
                file_data[label].append(serialized_object)    
                
            renderer.exit()
            tracker.exit()
            
        df = pd.DataFrame(file_data[label])
        df.to_csv(os.path.join(OUTPUT_PATH + "/" + label + "_blazepose" + '.csv'), index=False)    
        print(file_path + "\n\n")
            
        print("Time taken for category " + label + ": ")
        print((time.time() - file_start_time))
            
    print("Time taken for the program: ")
